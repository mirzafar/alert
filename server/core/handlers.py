...
env = Environment(loader=FileSystemLoader(f'{TEMPLATE_DIR}/templates'),
...
class BaseAPIView(TemplateHTTPView):
    decorators = [auth.login_required(user_keyword='user', name=['admin'], return_url='/admin/login/')]


class APIView(TemplateHTTPView):
    decorators = [auth.login_required(user_keyword='user', name=['client'], return_url='/login/')]
